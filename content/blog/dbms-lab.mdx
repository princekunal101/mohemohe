...
**Open the Cursor:** The cursor is opened with `open student_cursor`.
```sql title="mydb/postgres@PostgreSQL 17"
begin 
    open student_cursor;
```

**Fetch Data from the Cursor:** The data is fetching into `v_student_id` and `v_name` one row at a time within a loop.
```sql title="mydb/postgres@PostgreSQL 17"
loop
...
**Close the Cursor:** The cursor is closed with `close student_cursor`.
```sql title="mydb/postgres@PostgreSQL 17"
