...
10. **a.** Describe SQL. Explain different categories of SQL. **b.** What is aggregate function in SQL? Write SQL query for different aggregate function. **c.** What is a CLAUSE in terms of SQL? **d.** Explain embedded SQL and dynamic SQL in detail.  
...
4. Define partial functional dependency consider the following two sets of functional dependencies $$F = \{A\rightarrow C, AC \rightarrow D, E \rightarrow AD, E \rightarrow H \}$$ and $$G =\{A \rightarrow CD, E \rightarrow AH\}$$. Check whether or not they are equivalent.
...
7. Define minimal cover.Suppose a relation $$R(A,B,C)$$ has $$FD$$ set $$F=\{A \rightarrow B, B \rightarrow C, A \rightarrow C, AB \rightarrow B, AB \rightarrow C, AC \rightarrow B \}$$. Convert this $$FD$$ set into minimal cover.
8. A set of $$FD$$s for the relations $$R \{A,B,C,D,E,F\} $$ is $$AB\rightarrow C, C\rightarrow A, BC\rightarrow D, ABC\rightarrow B, BE\rightarrow C, EC\rightarrow FA, CF\rightarrow BD, D\rightarrow E$$. Find a minimum cover forth is set of $$FD$$s.
...
11. List all prime and non prime revealed in relation $$R\{A,B,C,D,E\}$$ with $$FD$$ set $$F=\{AB\rightarrow C, B \rightarrow E, C\rightarrow D\}$$.
...
13. Given the following set of $$FD$$s on a schema $$R\{V, W, X,Y,Z\}$$ $$Z\rightarrow V, W\rightarrow  YX, Y\rightarrow Z, V\rightarrow  WX$$ a state whether the following decomposition are lossless join decomposition or not. 
<br/>**a.** $$R1=\{V,W,X\}, R2=\{V,Y,Z\}$$ consider a relation $$R\{A,B,C,D,E\}$$ with set $$F\{A\rightarrow CD, C\rightarrow B, B\rightarrow AE\}$$ what are the primary attributes of this relation and decomposition the given relation in 3NF.
14. Consider the relation $$R\{A,B,C,D,E\}$$ with set $$F=\{A\rightarrow C, B\rightarrow D\}$$. decomposition this relation in 2NF.
15. Consider the universal relation $$R=\{A,B,C,D,E,F,G,H,I,J\}$$ and the set of functional dependencies $$F=\{AB\rightarrow C,A\rightarrow DE,B\rightarrow F,F\rightarrow GH, D\rightarrow IJ\}$$. what is the key for R? decompose R into 2NF and then 3NF relations.
16. What is highest normal form of the relation $$R\{X,Y,Z\}$$ with the set $$F=\{WY\rightarrow XZ, X\rightarrow Y\}$$.
17. Write the difference between 3NF and BCNF. Find normal form of relation $$R\{A,B,C,D,E\}$$ having $$FD$$ set $$F=\{A\rightarrow B, BC\rightarrow  E, ED\rightarrow A\}$$.
